...
import EditBuslinie from './EditBuslinie';
import CreateBuslinie from './CreateBuslinie';


class BuslinieTable extends Component {
    ...
    constructor(props) {
        ...
        this.handleChangeOpenCreateBuslinie = this.handleChangeOpenCreateBuslinie.bind(this);
        this.handleChangeOpenEditBuslinie = this.handleChangeOpenEditBuslinie.bind(this);
    };
    ...
    handleChangeOpenEditBuslinie = () => {
        if (this.state.selectedRow != null) {
            const open = true;
            const close = false;
            this.setState({ openEditBuslinie: open });
            this.setState({ openCreateBuslinie: close });
        }
    }

    handleChangeOpenCreateBuslinie = () => {
            const open = true;
            const close = false;
            this.setState({ openCreateBuslinie: open });
            this.setState({ openEditBuslinie: close });
    }
                ...
                <Paper sx={{ width: '100%', overflow: 'hidden' }}>
                    <Box sx={{ flexGrow: 1 }}>
                        <Toolbar sx={{ bgcolor: 'warning.light' }}>
                            ...
                            {customer
                                ? <div></div>
                                : <Button onClick={this.handleChangeOpenEditBuslinie}>
                                    Bearbeiten</Button>
                            }
                            {customer
                                ? <div></div>
                                : <Button onClick={this.handleChangeOpenCreateBuslinie}>
                                    Anlegen</Button>
                            }
                            {customer
                                ? <div></div>
                    ...
                    {(!this.state.openCreateBuslinie && !this.state.openEditBuslinie)|customer?
                        <Box>
                            ...
                            /></Box>
                        : <div></div>}
                    {this.state.openEditBuslinie && !customer?
                        <EditBuslinie EditSelectedRow={EditSelectedRow}/>
                        : <div></div>}
                    {this.state.openCreateBuslinie && !customer?
                        <CreateBuslinie/>
                        : <div></div>}
                </Paper>
            </Box>
